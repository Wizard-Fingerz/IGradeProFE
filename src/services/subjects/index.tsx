...
export const fetchAllSubjectsWithPagination = async (options: {
  pageIndex: number;
  pageSize: number;
  currentPage: number;
}) => {
  ...
  const response = await fetch(`${BASE_URL}/app/subjects/?page=${options.currentPage}`, {
    headers: {
...
export const fetchExaminationById = async (ExaminationId: any) => {
  const response = await fetch(`${BASE_URL}/Examination/${ExaminationId}/`, {
    headers: {
      Authorization: `Token ${token}`,
    },
  });
