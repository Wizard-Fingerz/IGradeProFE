...
import { fetchAllQuestionsWithPagination } from '../../services/questions';
import EditQuestionModal from '../../components/modals/questions/EditQuestionModal';
import DeleteQuestionModal from '../../components/modals/questions/DeleteQuestionModal';
...
const QuestionPage: React.FC = () => {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetchAllQuestionsWithPagination({
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'subject_name', headerName: 'Subject Name', flex: 1, type: 'string' },
    { field: 'subject_code', headerName: 'Subject Code', flex: 1, type: 'string' },
    {
      field: 'question_number', headerName: 'Question Number',
      flex: 1,
      type: 'string',
      renderCell: (params: GridRenderCellParams) => (
        <Link to={'#'}>
    ...
    {
      field: 'comprehension', headerName: 'Comprehension',
      flex: 1,
      type: 'string',
      renderCell: (params: GridRenderCellParams) => (
        <Link to={'#'}>
    ...
    { field: 'question', headerName: 'Question', flex: 1, type: 'string' },
    { field: 'examiner_answer', headerName: 'Examiner Answer', flex: 1, type: 'string' },
    { field: 'question_score', headerName: 'Question Score', flex: 1, type: 'number' },
    { field: 'is_optional', headerName: 'Is Optional', flex: 1, type: 'boolean', renderCell: (params: GridRenderCellParams) => (params.value ? 'Yes' : 'No') },
      ...
      renderCell: (params: GridRenderCellParams) => {
        ...
        return (
          <div>
            ...
            >
              <MenuItem>
                <EditQuestionModal questionID={params.row.id} btnText='Edit Question' />

              </MenuItem>
              <MenuItem>
                <DeleteQuestionModal questionID={params.row.id} btnText='Delete Question' />
              </MenuItem>
            </Menu>
          </div>
        );
    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', width: '100%', gap: 2 }}>
        <Button
          ...
          onClick={() => navigate('/exams/create-exam')}
        >
          Add Question
        </Button>
      ...
      <Grid container spacing={4} sx={{ mb: 5 }}>
        ...
        <Grid item xs={12} md={3}>
          <AnalyticsCard title="Total Questions" count={dataCount} />
        </Grid>
